...
async function createUsers() {
    ...
    while (users.length < totalUsers) {
        users.push({
            ...
            password,
            cpf_cnpj: faker.random.number(999999999999999),
            cep: faker.random.number(999999999999),
...
async function createProducts() {
    ...
    const productsPromisse = products.map(product => Product.create(product))
    productsIds = await Promise.all(productsPromisse)
